...
session_start();
error_reporting(0);
include('../connect.php'); // Adjust the path to connect.php
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    <table width="85%" align="center" class="table table-bordered table-striped" id="example1">
                      ...
                      <tbody>
                        <?php 
                          $sql = "SELECT * FROM admission WHERE ssce_details='3' AND room='room3/2' ORDER BY date_admission ASC"; // Updated SQL query
                          $result = $conn->query($sql);
                          $cnt = 1;
                          $totalFullnames = 0; // Initialize counter for fullnames
                          $femaleCount = 0; // Initialize counter for females

                          while($row = $result->fetch_assoc()) { 
                            $totalFullnames++; // Increment counter for each fullname
                            if ($row['sex'] == 'Female') { // Assuming 'Female' is the value for female students
                              $femaleCount++; // Increment counter for each female
                            }
                        ?>
                          <tr class="gradeX">
                    ...
                    <hr>
                    <!-- Display total number of fullnames and female count -->
                    <p>ຈຳນວນນັກຮຽນທັງໝົດ: <?php echo $totalFullnames; ?></p>
                    <p>ຍິງ: <?php echo $femaleCount; ?></p>
                  </div>
                </div>
              </td>
            </tr>
